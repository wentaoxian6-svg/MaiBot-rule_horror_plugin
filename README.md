...
- `current_model_index`: 当前使用的模型索引（从0开始）。当模型失败时会自动递增，超过范围会重置为0
...
2. 系统自动生成规则怪谈场景、场景结构和规则，并以长图形式展示
   ...
   - 场景结构文字长图：详细展示建筑类型、楼层布局、连接通道和特殊区域
   - 规则长图：清晰展示所有规则和通关条件
3. 阅读规则，分析隐藏的逻辑
...
- **理智崩溃机制** - 当理智值低于30时，场景描述会变得诡异、充满诱导性，直接与玩家对话试图颠覆逻辑
  - 理智值越低，理智崩溃效果越强烈
  - 视觉扭曲：红色涂鸦、斜线遮盖和模糊效果会根据理智值自动调整强度和覆盖面积
  - 文本扭曲：乱码符号插入、词语重复和字符错位会根据理智值自动调整频率和程度
  - 理智值为0时，行动长图将只显示场景描述和行动反馈，不显示任何状态栏，使用深红色大字体
...
├── environment_evolution.py  # 环境演化系统，包含NPC、时间推进、随机事件等
...
- 新增环境演化系统
  ...
  - **独立模型配置**：环境演化系统使用独立的模型配置，不与主系统混合
- 优化环境更新信息显示
  - 添加format_environment_update方法，优化环境更新信息的可读性
  - 清晰展示时间推进、环境变化和NPC行为
- 增强NPC交互体验
  - NPC主动交互：NPC会根据概率、位置和优先级主动与玩家交互
  - 身份引导：特定NPC会根据玩家身份提供特定指导和规则说明
  - 权限联动：身份变化时自动更新访问权限
  - 风险评估：对区域访问提供风险评估和负面后果
  - 后果触发：进入高风险区域可能触发NPC追杀、攻击等负面后果
- 新增配置选项
  - 添加[environment]配置节，支持独立的API地址、密钥、模型列表和温度参数
  - 环境演化系统使用独立配置，不影响主系统模型选择
- 代码优化
  - 提取公共方法，消除_start_new_game和_force_start_new_game之间的代码重复
  - 改进错误处理和日志输出

### v1.4.1
- 新增模型列表功能
  - 将单个模型配置改为模型列表配置，支持按优先级排序多个LLM模型
  - 当当前模型调用失败时，系统会自动切换到列表中的下一个模型
  - 自动更新当前模型索引，记录成功使用的模型
  - 提供详细的模型切换日志，方便排查问题
  - 支持循环切换，当所有模型都尝试后会回到第一个模型
- 新增手动清理存档功能
  - 添加 `/rg 清理存档` 命令，用于手动清理已结束的存档及其关联图片
  - 自动识别并删除所有已结束的存档文件（game_active=false）
  - 同时删除存档关联的所有图片文件（包括剧情导入、场景结构、规则、行动结果、结局等长图）
  - 提供详细的清理结果反馈，显示删除的存档数量和图片数量
  - 若无已结束存档则提示用户
