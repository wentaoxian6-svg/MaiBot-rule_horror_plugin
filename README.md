...
5. 如果启用环境演化系统，则需要使用支持结构化输出的模型（如官网的glm）。
6. 启用环境演化系统后，建议主系统使用gemini-3(flash或者pro)等支持结构化输出的长上下文高智商模型。
...
# 是否允许直接发送自然语言触发行动（默认关闭，避免误触发/剧透）
...
- `enable_natural_language_action`: 是否启用“自然语言直接行动”（建议新手保持关闭，使用 `/rg 行动` 更稳定）
...
**注意**：自然语言输入默认关闭。需要在 `config.toml` 中设置 `plugin.enable_natural_language_action = true` 才会把“非 `/rg` 消息”当作行动处理。

**提示**：开启后，自然语言输入仍需要包含行动关键词（如：走、去、看、拿、用、喝、吃、休息等），否则会被忽略。

**沉浸式结束**：在游戏进行中，你可以直接发送 `结束` 或 `结束游戏` 来结束并判定结局（等同 `/rg 结束`）。
