...
| `/rg 道具` | 查看当前道具列表 |
| `/rg 道具 <名称>` | 查看指定道具详情 |
| `/rg 物品栏` | 查看当前道具列表（同 `/rg 道具`） |
| `/rg 背包` | 查看当前道具列表（同 `/rg 道具`） |
| `/rg 线索` | 查看当前已知线索 |
| `/rg 线索 <名称>` | 查看指定线索详情 |
...
| `/rg 推理 <内容>` | 记录推理内容 |
| `/rg 记录规则 <内容>` | 记录推理出的规则 |
| `/rg 提示` | 基于规则笔记推断进度，LLM 自主决定引导方向 |
| `/rg 提示 规则` | 软偏好：希望往规则方向引导（仅作参考） |
| `/rg 提示 线索` | 软偏好：希望往线索方向引导（仅作参考） |
...
- `/rg 提示` 会基于玩家已记录的规则笔记对比后台真实规则与隐藏真相，推断推理进度后给出非直白引导
- 玩家参数（"规则"/"线索"）为**软偏好**，与进度判断冲突时以进度判断为主
- 多人模式下 LLM 可见所有存活玩家的规则笔记用于全队进度推断
- 玩家收到的消息只包含 `hint` + `next_action`，进度标签与引导方向仅写入日志
...
- 通过 `asyncio.Semaphore(8)` 做全局并发限流
